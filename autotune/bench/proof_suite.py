...
async def run_model_benchmark(
    ...
    with Progress(
        ...
        if model_info.is_valid():
            ...
            console.print(
                "[dim yellow]  Could not fetch architecture info — "
                "KV estimates will show N/A[/dim yellow]"
            )
...
def print_quick_verdict(report: ModelReport, console: Console) -> None:
    ...
    lines += [
        ...
        "",
        "  [dim]Run [bold]--complete[/bold] for all 5 prompts, growth test, "
        "and per-prompt stats tables.[/dim]",
    ]
