...
# ── Package module — no sys.path manipulation needed ────────────────────────
_ROOT = Path(__file__).parent.parent.parent  # repo root (dev) or site-packages root
...
from autotune.api.ctx_utils import estimate_messages_tokens, estimate_tokens
...
QUICK_TASK_IDS  = ["code_debugger", "extended_session"]
# Full mode: all 5 tasks — enable with --full
FULL_TASK_IDS   = ALL_TASK_IDS
...
def _generate_markdown_report(
    ...
    for model_id, task_stats_list in model_results.items():
        ...
        for ts in task_stats_list:
            lines += [
                f"#### {ts['task_id']}",
                "",
                "| KPI | Raw | autotune | Δ | Cohen's d | p | Improved? |",
                "|-----|-----|----------|---|-----------|---|-----------|",
            ]
    ...
    lines += [
        ...
        "",
        "*Report generated by autotune agent-bench v1.0*",
    ]
...
async def _async_main(args: argparse.Namespace) -> int:
    ...
    if args.tasks:
        ...
        task_ids = set(FULL_TASK_IDS)
    else:
        task_ids = set(QUICK_TASK_IDS)   # default: 2 focused tasks, ~8-12 min
    tasks     = [t for t in all_tasks if t.task_id in task_ids]
    if not tasks:
        console.print(f"[red]No tasks matched: {task_ids}[/red]")
        return 1

    # Default trials: 2 per condition (enough to show the TTFT trend clearly).
    # --full bumps to 5 for statistical rigour.
    if hasattr(args, "trials") and args.trials != 5:
        n_trials = args.trials          # explicit user override
    elif full_mode:
        n_trials = 5
    else:
        n_trials = 2                    # fast default
